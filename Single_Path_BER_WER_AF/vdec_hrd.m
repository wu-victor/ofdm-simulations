% Viterbi decoder using hard decisions.

function out = vdec_hrd(in, n) % length(in) should be 3*(n+2).
...
% s0 -> s1.
p1mtr = sum(abs(rxbts - [1 1 1]));
p1bts = [1];

% s0 -> s0.
p0mtr = sum(abs(rxbts - [0 0 0]));
p0bts = [0];
...
% s1 -> s3.
p3mtr = p1mtr_old + sum(abs(rxbts - [0 0 0]));
p3bts = [1 1];

% s1 -> s2.
p2mtr = p1mtr_old + sum(abs(rxbts - [1 1 1]));
p2bts = [1 0];

% s0 -> s1.
p1mtr = p0mtr_old + sum(abs(rxbts - [1 1 1]));
p1bts = [0 1];

% s0 -> s0.
p0mtr = p0mtr_old + sum(abs(rxbts - [0 0 0]));
p0bts = [0 0];
...
for ti = 3 : n
    ...
    % s3 -> s3.
    mtr1 = sum(abs(rxbts - [1 1 0]));
    % s1 -> s3.
    mtr2 = sum(abs(rxbts - [0 0 0]));
    ...
    % s3 -> s2.
    mtr1 = sum(abs(rxbts - [0 0 1]));
    % s1 -> s2.
    mtr2 = sum(abs(rxbts - [1 1 1]));
